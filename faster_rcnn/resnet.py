...
class ResNet152(nn.Module):
    ...
    def __init__(self, endlayer='', block=Bottleneck, layers=[3, 8, 36, 3], num_classes=1000):
    ...
    def forward(self, x, endlayer = 'layer3'):
        if endlayer == 'layer3':
            x = self.conv1(x)
            x = self.bn1(x)
            x = self.relu(x)
...
if __name__ == '__main__':
    resnet = ResNet152()
    resnet.load_from_pth('/disk2/data/pytorch_models/resnet152-b121ed2d.pth')
